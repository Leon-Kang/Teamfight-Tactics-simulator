...
def action(champion):
    if len(champion.enemy_team()) > 0 and not champion.stunned:
        ...
        if champion.millis() > 0 and champion.champion and not champion.ability_requires_target and champion.maxmana > 0 and champion.mana >= champion.maxmana and not (
                champion.disarmed and not stats.ABILITY_WHILE_DISARMED[
            champion.name]) and champion.millis() > champion.castMS + stats.MANALOCK[champion.name]:
            champion.ability()
            ...
            if champion.target:
                ...
                elif champion.millis() > 0 and not (
                        champion.ability_active and not stats.ATTACK_WHILE_ABILITY_ACTIVE[champion.name]):
                    if champion.champion and 0 < champion.maxmana <= champion.mana and not (
                            champion.disarmed and not stats.ABILITY_WHILE_DISARMED[
                        champion.name]) and champion.millis() > champion.castMS + stats.MANALOCK[champion.name]:
                        champion.ability()
...
def find_target(c):
    ...
    for y in coordinates:
        for x in y:
            ...
            if x is not None and x.team is not c.team and x.champion and x.health > 0:
                ...
                dist = (abs(c_coords['x'] - x_coords['x']) + abs(c_coords['y'] - x_coords['y']) + abs(
                    c_coords['z'] - x_coords['z'])) / 2
    ...
    if current_target['champion']:
        ...
        if c.target != old_target:
            c.print(' has a new target: ' + '{:<8}'.format(c.target.team) + '{:<8}'.format(
                c.target.name) + '  [{}, {}]'.format(c.target.y, c.target.x))
...
def distance(champion1, champion2, objects):
    ...
    dist = (abs(c1_coords['x'] - c2_coords['x']) + abs(c1_coords['y'] - c2_coords['y']) + abs(
        c1_coords['z'] - c2_coords['z'])) / 2
    return dist
...
def leap_to_back_line(champion, data):
    ...
    if target_hex:
        ...
        coordinates[champion.y][champion.x] = champion
