    ...
    if 'hextech_gunblade' in champion.items:
        ...
        if healable > heal:
            ...
            champion.add_que('shield', 0, None, None,
                             {'amount': heal, 'identifier': shield_identifier, 'applier': champion,
                              'original_amount': heal},
...
def last_whisper(champion, target):
    ...
    if 'last_whisper' in champion.items:
        ...
        if len(last_activation) == 0 or (millis - last_activation[0][1]) > length:
            ...
            target.add_que('change_stat', length, None, 'armor', None,
                           {'vi': item_stats.item_armor_decrease['last_whisper']})
...
def locket_of_the_iron_solari(champion):
    ...
    for holder in holders:
        ...
        hexes = []
        if holder.x >= 2:
            hexes.append(coords[holder.y][holder.x - 2])
        if holder.x >= 1:
            hexes.append(coords[holder.y][holder.x - 1])
        hexes.append(coords[holder.y][holder.x])
        if holder.x <= 5:
        ...
        for h in hexes:
            if h and h.team == holder.team and h.champion:
                ...
                h.add_que('shield', -1, None, None,
                          {'amount': shield_size, 'identifier': shield_identifier, 'applier': holder,
                           'original_amount': shield_size}, {'increase': True, 'expires': shield_length})
...
def shroud_of_stillness(champion):
    ...
    for holder in holders:
        ...
        if champion.y % 2 == 0:
            ...
            if champion.x <= 5:
                affect_odd_x.append(champion.x + 1)
            if champion.x >= 1:
                affect_even_x.append(champion.x - 1)
        ...
        if inx == -1:
            ...
            if current_count == item_stats.item_activate_every_x_attacks['statikk_shiv']:
                ...
                for t in targets:
                    ...
                    true_damage = 0
                    if t.shield_amount() > 0:
                        true_damage += item_stats.true_damage['statikk_shiv'] * item_amount
...
def duelists_zeal(champion): origin_class.amounts['duelist'][champion.team] += len(
    list(filter(lambda x: x == 'duelists_zeal', champion.items)))


def elderwood_heirloom(champion): origin_class.amounts['elderwood'][champion.team] += len(
    list(filter(lambda x: x == 'elderwood_heirloom', champion.items)))


def mages_cap(champion): origin_class.amounts['mage'][champion.team] += len(
    list(filter(lambda x: x == 'mages_cap', champion.items)))


def mantle_of_dusk(champion): origin_class.amounts['dusk'][champion.team] += len(
    list(filter(lambda x: x == 'mantle_of_dusk', champion.items)))


def sword_of_the_divine(champion): origin_class.amounts['divine'][champion.team] += len(
    list(filter(lambda x: x == 'sword_of_the_divine', champion.items)))


def vanguards_cuirass(champion): origin_class.amounts['vanguard'][champion.team] += len(
    list(filter(lambda x: x == 'vanguards_cuirass', champion.items)))


def warlords_banner(champion): origin_class.amounts['warlord'][champion.team] += len(
    list(filter(lambda x: x == 'warlords_banner', champion.items)))


def youmuus_ghostblade(champion): origin_class.amounts['assassin'][champion.team] += len(
    list(filter(lambda x: x == 'youmuus_ghostblade', champion.items)))
