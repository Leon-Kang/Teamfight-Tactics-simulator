...
def jhin(champion, target):
    ...
    if jhin_shots[index][1] == stats.ACTIVATE_EVERY_X_ATTACKS[champion.name]:
        ...
        return {'damage': (stats.ACTIVE_DMG_PERCENT[champion.name][champion.stars] * champion.SP - 1) * champion.AD,
                'true_damage': False, 'crit_random': None, 'dodge_random': None}
...
def kalista(champion, target):
    if target:
        ...
        spear_damage = stats.ACTIVE_TARGET_HEALT_THRESHOLD[champion.name][champion.stars] * target.max_health * \
                       kalista_targets[index][2]
...
def vayne(champion, target):
    ...
    if vayne_targets[index][2] == stats.ACTIVATE_EVERY_X_ATTACKS[champion.name]:
        ...
        champion.print(' active triggered')
        return {'damage': stats.ACTIVE_DMG[champion.name][champion.stars] * champion.SP, 'true_damage': True,
                'crit_random': None, 'dodge_random': None}
                ...
                if o != champion:
                    ...
                    for t in origin_class_stats.origin_class[o.name]:
                        if t in ww_traits:
                            ...
                            o.add_que('change_stat', stats.ABILITY_LENGTH[champion.name], None, 'AS', None,
                                      {'ezreal': as_gain})
                            break
