...
class champion:
    def __init__(self, name, stars, team, y, x,
                 itemlist=[], overlord=None, sandguard_overlord_coordinates=None, chosen=False):
    ...
    def attack(self, bonus_dmg=0, target=None, item_attack=False, trait_attack='', set_AD=None):
        ...
        if target or self.target:
            ...
            if (not (self.overlord and (self.overlord.stunned or self.overlord.disarmed or self.overlord.blinded)) or (
                    self.overlord and self.overlord.stunned and not self.overlord.champion)):

                # enforcing ashe ult here
                if (self.name == 'ashe' and self.ability_active and not trait_attack and len(
                        self.enemy_team()) > 0 and not item_attack):
    ...
    def spell(self, target, dmg, true_dmg=0, item_damage=False, burn_damage=False, trait_damage=False):
        ...
        if not ('trap_claw' in target.items and not item_damage and not burn_damage):  # trap_claw
            ...
            if (
                    'jeweled_gauntlet' in self.items and crit_random < self.crit_chance and self != target and not 'bramble_vest' in target.items and not item_damage):
            ...
            if (
                    crit_random < self.crit_chance and self != target and not 'bramble_vest' in target.items and not item_damage and crit_string == '' and origin_class.is_trait(
                self, 'assassin') and origin_class.get_origin_class_tier(self.team, 'assassin') > 0):
                damage *= self.crit_damage
                crit_string = 'crit'
            ...
            if target in eval(enemy_team):
                ...
                self.print(' deals ' + '{:<8}'.format(enemy_team) + ' ' + '{:<13}'.format(
                    target.name) + '{:<5}--> {:<8}   shield {:<5}--> {:<5} {}{}{}{}'.format(ceil(target.health), ceil(
                    target.health - damage), ceil(shield_old), ceil(target.shield_amount()), crit_string, burn_string,
                                                                                            item_string, trait_string))
