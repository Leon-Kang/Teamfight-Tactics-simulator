...
def annie(champion):
    ...
    for n in neighbors:
        if (n != leave_out and field.coordinates[n[0]][n[1]] and field.coordinates[n[0]][n[1]].team != champion.team and
                field.coordinates[n[0]][n[1]].champion):
...
def cassiopeia(champion):
    ...
    for c in cone:
        ...
        if 0 <= c[0] <= 7 and c[1] >= 0 and c[1] <= 6:
            h = coords[c[0]][c[1]]
            if h and h.team != champion.team and h.champion and h not in already_targeted:
                ...
                if (not ('quicksilver' in h.items and champion_functions.MILLIS() <= item_stats.item_change_length[
                    'quicksilver'])):
                    h.print(' {} {} --> {}'.format('stunned', h.stunned, True))
                    h.stunned = True
                    h.clear_que_stunned_removal()

                    h.add_que('change_stat', stats.ABILITY_STUN_DURATION[champion.name][champion.stars], None,
                              'stunned', False)
                else:
    ...
    for n in neighbors:
        ...
        if c and c.team != champion.team and c.champion:
            ...
            champion.add_que('heal', -1, None, None, (
                    stats.ABILITY_HEAL[champion.name][champion.stars] / stats.ABILITY_SLICES[
                champion.name]) * champion.SP)
...
def kayn(champion, data={'redash': False}):
    if not champion.stunned:
        ...
        for e in enemies_in_range:
            ...
            if ('shadow_assassin' in champion.items and champion_functions.MILLIS() < stats.ABILITY_EXTRA_DAMAGE_LENGTH[
                champion.name]):
                ...
                if end_point[:2] != end_point_original:
                    ...
                    while not kick_coords:
                        if len(line) == 0:
                            ...
                            if (not (
                                    champion.target.y == 0 or champion.target.y == 7 or champion.target.x == 0 or champion.target.x == 6)):
                                deal = True
...
def riven_helper(champion, data):
    ...
    own_shields = list(filter(lambda x: (
            x['applier'] == champion and x['original_amount'] == stats.SHIELD_AMOUNT[champion.name][
        champion.stars] * champion.SP), champion.shields))
...
def sett(champion):
    ...
    if (len(champion.enemy_team()) > 0):
        ...
        two_from_champion = field.hexes_distance_away(champion.y, champion.x, 2, True)
        two_from_n0 = field.hexes_distance_away(one_distance_neighbors[0][0][0], one_distance_neighbors[0][0][1], 2,
                                                True)
        two_from_n1 = field.hexes_distance_away(one_distance_neighbors[1][0][0], one_distance_neighbors[1][0][1], 2,
                                                True)
...
def sylas(champion):
    ...
    distance = field.distance({'y': champion.y, 'x': champion.x}, {'y': champion.target.y, 'x': champion.target.x},
                              False)
    smash_line = []
...
def teemo(champion):
    ...
    if (
            not ((origin_class.get_origin_class_tier(champion.team, 'mage') > 0 and origin_class.is_trait(champion,
                                                                                                          'mage')))):
        for i in range(0, 5):
            que = champion.que_return()
            for q in que:
                for t in targets:
                    if (q[1] is champion and q[0] == 'execute_function' and q[3][0] == teemo_ability and 'target' in
                            q[3][1] and q[3][1]['target'] == t):
                        que.remove(q)
            champion.que_replace(que)
