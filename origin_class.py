...
amounts = {
    'cultist': {'blue': 0, 'red': 0},
    # in champion.py: champion object, champion.champion_functions.py: add_damage_dealt(), attack(), ability.py: default_ability_calls, galio()
    ...
    'assassin': {'blue': 0, 'red': 0},
    # in origin_class.py: total_origin_class(), field.py: leap_to_back_line(), champion.py: spell()
...
def divine_helper(champion, divine_tier):
    ...
    champion.add_que('change_stat', origin_class_stats.length['divine'][divine_tier], None, 'receive_decreased_damage',
                     old_damage_receiving)
        ...
        if tier > 0:
            for c in teams[t]:
                if is_trait(c, 'enlightened'):
                    items.change_stat(c, 'mana_generation', origin_class_stats.mana_generation['enlightened'][tier],
                                      'enlightened')
            ...
            for c in teams[t]:
                if is_trait(c, 'exile'):
                    ...
                    if applicable:
                        ...
                        c.add_que('shield', -1, None, None,
                                  {'amount': shield_size, 'identifier': shield_identifier, 'applier': c,
                                   'original_amount': shield_size}, {'increase': True})
...
def the_boss_helper(champion, data):
    ...
    if len(champion.own_team()) == 1 or champion.pumped_up:
        ...
        for i, h in enumerate(hexes):
            d = field.distance({'y': corner[champion.team][0], 'x': corner[champion.team][1]}, {'y': h[0], 'x': h[1]},
                               False)
            hexes[i].append(d)
                ...
                if is_trait(c, 'keeper'):
                    ...
                    for h in hexes:
                        ...
                        if u and u.team == c.team and u.champion:
                            ...
                            u.add_que('shield', -1, None, None,
                                      {'amount': shield, 'identifier': shield_identifier, 'applier': c,
                                       'original_amount': shield}, {'increase': True, 'expires': length})
        ...
        for i in range(0, bounces):
            ...
            if len(possible_targets) > 0:
                ...
                if spell:
                    ...
                    champion.attack(true_damage, next_target, False, 'sharpshooter',
                                    champion.AD * damage_multiplier ** (i + 1))
